...
The severity can be `error`, `warning`, or `info`. The Git hook blocks errors and warnings.

The profile has two modes:

| Mode | Behavior |
|---|---|
| `pragmatic` | Blocks high-confidence problems. Reports modal verbs and complex tenses as notes. |
| `strict` | Applies the configured severity to every Simple English rule. |

Use a rule entry to change one result. For example, this entry makes modal verbs block a commit in pragmatic mode:
